...
pub fun main(address: Address, series: String) : {String:Bool} {

    var series_index = -1
    let series_array = ["Omnist","Charles-Mastery"]

    for index, item in series_array {
      if item == series {
        series_index = index
      }
    }
    ...
    let mainRef = getAccount(address).getCapability<&{SoulMadeMain.CollectionPublic}>(SoulMadeMain.CollectionPublicPath).borrow() ?? panic("Could not borrow the reference")
    ...
    for setName in preDefinedSets.keys {
      ...
      for mainId in mainRef.getIDs(){
        ...
        var mainNFT = mainRef.borrowMain(id: mainId)
        if mainNFT.mainDetail.series != series {
          continue
        }

        var allComponents : [SoulMadeComponent.ComponentDetail] = mainNFT.getAllComponentDetail().values
        ...
        for component in allComponents{
          ...
          if !desiredComponentIpfsHashList.contains(ipfsHash) || alreadyClaimedComponentIdList.contains(componentNftId) {
            res = false
            break
          }
...
const checkSoulMadeComplete = async (account,series)  => {
    try {
